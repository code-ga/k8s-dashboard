...
import { ArrowLeft, Database, Plus, Trash2, Maximize2 } from "lucide-react";
...
export const Route = createFileRoute("/dashboard/cluster/$id/pvcs/")({
	component: ClusterPVCs,
});

function ClusterPVCs() {
	const { id } = useParams({ from: "/dashboard/cluster/$id/pvcs/" });
	...
	const deletePvc = async (pvcId: number) => {
		...
		const res = await api.api.pvcs({ clusterId: id })({ id: pvcId.toString() }).delete();
		if (res.error) {
	...
	if (isLoading) return <div className="p-8 text-center text-muted-foreground animate-pulse font-medium tracking-tight">Accessing cluster storage...</div>;
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-4">
					...
					<div>
						<h2 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Persistent Volume Claims</h2>
					...
					<Table>
						<TableHeader className="bg-muted/50 border-b border-border/50">
							<TableRow>
								...
								<TableHead className="font-semibold py-4">Storage Class</TableHead>
								<TableHead className="text-right px-6 font-semibold py-4">Actions</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							{pvcs?.map((pvc) => (
								<TableRow key={pvc.id} className="hover:bg-accent/10 transition-colors group">
									<TableCell className="font-medium px-6 py-4 flex items-center gap-3">
										<div className="p-2 bg-blue-100 rounded-lg group-hover:bg-blue-200 transition-colors">
											<Database className="h-4 w-4 text-blue-600" />
										</div>
										<span className="font-semibold text-foreground/90">{pvc.name}</span>
									</TableCell>
									...
									<TableCell>
										<span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ring-1 ring-inset ${
											pvc.phase === 'Bound' ? 'bg-green-100 text-green-700 ring-green-600/20' : 
											pvc.phase === 'Pending' ? 'bg-yellow-100 text-yellow-700 ring-yellow-600/20' : 
									...
									<TableCell className="font-mono text-[10px] text-muted-foreground selection:bg-blue-100">
										{pvc.volumeName || <span className="opacity-30">Pending...</span>}
									</TableCell>
									...
									<TableCell className="text-right px-6">
										<div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200">
											{can("pvc:update") && pvc.phase === 'Bound' && (
												<Button
													variant="ghost"
													size="icon"
							...
							{(!pvcs || pvcs.length === 0) && (
								<TableRow>
									<TableCell colSpan={7} className="text-center py-24 text-muted-foreground/50">
										<div className="flex flex-col items-center justify-center space-y-4">
											<div className="p-4 bg-muted/20 rounded-full animate-pulse">
												<Database className="h-12 w-12 opacity-20" />
											</div>
											<div className="space-y-1">
												<p className="text-xl font-semibold tracking-tight text-foreground/70">No Storage Claims Found</p>
												<p className="max-w-[250px] mx-auto text-sm opacity-60">
													Provisions persistable disk space for your pods by creating a new claim.
												</p>
											</div>
										</div>
									</TableCell>
